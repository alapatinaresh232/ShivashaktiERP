...
namespace SSCRM
{
    public partial class TrainingPrgEmpDetails : Form
    {
        SQLDB objSQLdb = null;
        public TrainingPlanningDetails objTrainingPlanningDetails;

        public TrainingPrgEmpDetails()
        {
            InitializeComponent();
        }

        private void TrainingPrgEmpDetails_Load(object sender, EventArgs e)
        {
            FillCompanyData();
            //FillDepartmentData();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (CheckData())
            {
                dgvEmpDetl = ((TrainingPlanningDetails)objTrainingPlanningDetails).gvEmpDetails;
        ...
        public void AddEmpDetailsToGrid(DataGridView dgvEmpDetl)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < clbEmployees.Items.Count; i++)
                {
                    ...
                    {
                         IsItemExisted = false;
                         if (((TrainingPlanningDetails)objTrainingPlanningDetails).gvEmpDetails.Rows.Count > 0)
                        {
                            for (int j = 0; j < ((TrainingPlanningDetails)objTrainingPlanningDetails).gvEmpDetails.Rows.Count; j++)
                            {
                                if (((NewCheckboxListItem)(clbEmployees.Items[i])).Tag.Equals(((TrainingPlanningDetails)objTrainingPlanningDetails).gvEmpDetails.Rows[j].Cells["Ecode"].Value.ToString()))
                                {
                                    IsItemExisted = true;
                                }
