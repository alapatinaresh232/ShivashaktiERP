...
namespace SSCRM
{
    public partial class SPInvoice : Form
    {
        ...
        private bool isSPInvExist = true;
        private string sSPDocMonth = "";
        private string strCompCode = "", strBranch = "", strOrderNo = "", strFinYear = "";
        public GCGLAdvances objGCGLAdvances;
        ...
        public SPInvoice(string sCompany, string sBranCode, string sFinYear, string sOrderNo)
        {
            InitializeComponent();
            strCompCode = sCompany;
            strBranch = sBranCode;
            strFinYear = sFinYear;
            strOrderNo = sOrderNo;
        }

        private void SPInvoice_Load(object sender, EventArgs e)
        {
            //txtOrderNo.Focus();
            ...
            txtSpInvoiceNo.Text = GenerateNewStockPointInvNo().ToString();
            ...
            cbCompany.Focus();
           // txtOrderNo.Focus();
            if (strCompCode.Length > 0 && strBranch.Length > 0 && strOrderNo.Length > 0)
            {
                cbCompany.SelectedValue = strCompCode;
                cbCompany_SelectedIndexChanged(null, null);
                cbBranch.SelectedValue = strBranch;
            ...
            {
                strFinYear = CommonData.FinancialYear;
        ...
        private void FillCompanyData()
        {
            ...
            try
            {
                strCmd = "SELECT CM_COMPANY_CODE,CM_COMPANY_NAME FROM COMPANY_MAS WHERE ACTIVE='T' ORDER BY CM_COMPANY_NAME";
        ...
        private void FillBranchData()
        {
            ...
            try
            {
                if (cbCompany.SelectedIndex > 0)
                {
                    ...
                    string strCommand = "SELECT BRANCH_NAME BranchName,BRANCH_CODE+'@'+STATE_CODE+'@'+sm_state BranCode " +
                                        " FROM BRANCH_MAS " +
                                        " INNER JOIN state_mas ON sm_state_code=STATE_CODE "+
                                        " WHERE COMPANY_CODE='" + cbCompany.SelectedValue.ToString() +
                                        "'and ACTIVE='T' and branch_type='BR' Order by BRANCH_NAME ";
        ...
        private void FillEmployeeData()
        {
            ...
            if (cbCompany.SelectedIndex > 0 && cbBranch.SelectedIndex > 0)
            {
                try
                {
                    dsEmp = objData.GetEcodeList(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], txtDocMonth.Text);
        ...
        private void FillProductData()
        {
            ...
            try
            {
                Cursor.Current = Cursors.WaitCursor;
                ds = new DataSet();
                ds = objData.GetProductDataSet(cbCompany.SelectedValue.ToString(),cbBranch.SelectedValue.ToString().Split('@')[0]);
                DataTable dt = ds.Tables[0];
                int intRow = 1;
                ...
                if (CheckData() == true)
                {
                    ...
                    if (intSaved > 0)
                    {
                        if (intSaved > 0 ) //&& IsModifyBulletIn == true)
                        {
                            ...
                            DialogResult result = MessageBox.Show("Do you want to Print this invoice?",
                                                 "CRM", MessageBoxButtons.YesNo);
                            if (result == DialogResult.Yes)
                            {
                                ReportViewer childReportViewer = new ReportViewer(CommonData.CompanyCode,CommonData.BranchCode, CommonData.FinancialYear, "ALL",txtSpInvoiceNo.Text, txtSpInvoiceNo.Text, "SP_INVOICE");
                       ...
                       // txtOrderNo.Focus();
                        ...
                        {
                            objGCGLAdvances.FillGCGLAdvances();
                            this.Close();
                            this.Dispose();
                        }
        ...
        {
            objSQLDB = new SQLDB();
            string sqlText = " exec SSCRM_SERVICE_TNA_AUD '" + cbCompany.SelectedValue.ToString() +
        ...
        private Int64 GenerateNewStockPointInvNo()
        {
            ...
            try
            {
                ...
                nSPInvNo = objSPDB.GenerateInvNoForSPInvoice(CommonData.BranchCode);
                objSPDB = null;
            }
            catch (Exception ex)
            {
        ...
        private Int64 GenerateNewInvoiceNo()
        {
            ...
            if (cbCompany.SelectedIndex > 0 && cbBranch.SelectedIndex > 0)
            {
                try
                {
                    ...
                    objData = new InvoiceDB();
                    intNewInvNo = objData.GenerateInvoiceNo(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0]);
                    objData = null;
                }
                catch (Exception ex)
                {
        ...
        private bool CheckData()
        {
            ...
            if (cbCompany.SelectedIndex == 0)
            {
                MessageBox.Show("Please Select Company", "Invoice", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                blValue = false;
                cbCompany.Focus();
                return blValue;
            }
            ...
            {
                MessageBox.Show("Please Select Branch", "Invoice", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                blValue = false;
                cbBranch.Focus();
                return blValue;
            }
            ...
            if (IsSaleOrder == false)
            {
                ...
                try
                {
                    dtOrderStatus = objData.Get_OrderSheetIssueDetails_ByOrderNo(cbCompany.SelectedValue.ToString(),cbBranch.SelectedValue.ToString().Split('@')[0], txtDocMonth.Text.ToUpper(), Convert.ToInt32(txtOrderNo.Text).ToString("00000")).Tables[0];
                }
                catch { return false; }
        ...
        private int SaveInvoiceHeadData()
        {
            ...
            try
            {
                if (CheckOrderNoExistInSPInvoice(0,Convert.ToInt32(txtOrderNo.Text).ToString("00000")) == false)
                    txtSpInvoiceNo.Text = GenerateNewStockPointInvNo().ToString();
                ...
                if (IsModifyInvoice == false)
                {
                    ...
                    if (Convert.ToInt32(txtInvoiceNo.Text) != 0)
                    {
                        sqlText = " INSERT INTO Sales_Inv_Head(sih_company_code, sih_state_code, sih_branch_code, sih_fin_year" +
                                    ...
                                    " VALUES ('" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                    "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] + "' ,'" + CommonData.FinancialYear + "'," + txtInvoiceNo.Text + 
                                    ", '" + Convert.ToDateTime(dtpInvoiceDate.Value).ToString("dd/MMM/yyy") + "' " +
                        ...
                        sqlText += " INSERT INTO SALES_INV_BULTIN_HEAD(sibh_company_code, sibh_state_code, sibh_branch_code, sibh_fin_year"+
                                    ...
                                    ", SIBH_ADVANCE_AMOUNT, SIBH_RECEIVED_AMOUNT, SIBH_CREDIT_SALE, SIBH_FIELD_SUP_BY, SIBH_ORDER_DATE)" +
                                     " SELECT '" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                     "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] +"' , '" + CommonData.FinancialYear + 
                                     "'," + txtInvoiceNo.Text + ", '" + Convert.ToDateTime(dtpInvoiceDate.Value).ToString("dd/MMM/yyy") + "' " +
                                     ", '99999', '" + Convert.ToInt32(txtOrderNo.Text).ToString("00000") + "', '" + strECode + 
                                     "', 'Ppattern', '" + CommonData.DocMonth +
                                     ...
                                     " WHERE sibh_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                                     "' AND sibh_fin_year='" + CommonData.FinancialYear +
                                     "' AND sibh_order_number='" + Convert.ToInt32(txtOrderNo.Text).ToString("00000") + "')";

                        if (CheckOrderNoExistInSPInvoice(0, Convert.ToInt32(txtOrderNo.Text).ToString("00000")) == false)
                        {
                            sqlText += "INSERT INTO SALES_INV_SP_HEAD(SISH_COMPANY_CODE,SISH_BRANCH_CODE,SISH_FIN_YEAR,SISH_DOC_MONTH " +
                                       ",SISH_INVOICE_NO,SISH_ORDER_NO,SISH_SP_BRANCH_CODE,SISH_SP_FIN_YEAR,SISH_SP_DOC_MONTH " +
                                       ",SISH_SP_SP_INVOICE_NO,SISH_CREATED_BY,SISH_CREATED_DATE " +
                                       ")VALUES('" + cbCompany.SelectedValue.ToString() + "','" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                                       "','" + CommonData.FinancialYear + "','" + txtDocMonth.Text.ToUpper() + "'," + txtInvoiceNo.Text +
                                       ",'" + Convert.ToInt32(txtOrderNo.Text).ToString("00000") + "','" + CommonData.BranchCode +
                                       "','" + CommonData.FinancialYear + "','" + CommonData.DocMonth + "'," + txtSpInvoiceNo.Text +
                                       ",'" + CommonData.LogUserId + "',getdate())";
                        }
                        else
                        {
                            sqlText += "UPDATE SALES_INV_SP_HEAD SET SISH_MODIFIED_BY='" + CommonData.LogUserId +
                                      "', SISH_MODIFIED_DATE=getdate() " +
                                      " WHERE SISH_SP_BRANCH_CODE='" + CommonData.BranchCode +
                                      "' and SISH_SP_FIN_YEAR='" + CommonData.FinancialYear +
                                      "' and SISH_SP_SP_INVOICE_NO=" + txtSpInvoiceNo.Text + "";
                        }

                        sqlText += " DELETE from Sales_Inv_Detl_FREE" +
                                " WHERE sidf_company_code='" + cbCompany.SelectedValue.ToString() +
                                    "' AND sidf_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                        ...
                        sqlText += " DELETE from SALES_INV_BULTIN_DETL" +
                             " WHERE sibd_company_code='" + cbCompany.SelectedValue.ToString() +
                                 "' AND sibd_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                        ...
                        sqlText += " UPDATE SALES_INV_BULTIN_HEAD set sibh_invoice_date='" + Convert.ToDateTime(dtpInvoiceDate.Value).ToString("dd/MMM/yyy") +
                                   ...
                                   "  AND sibh_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                                   "' AND sibh_fin_year='" + CommonData.FinancialYear.ToString() +
                                   "' AND sibh_company_code='" + cbCompany.SelectedValue.ToString() +
                ...
                {
                    //string sqlDelete = "";
                    sqlText = " DELETE FROM SERVICES_TNA WHERE TNA_COMPANY_CODE = '" + cbCompany.SelectedValue.ToString() +
                                    "' AND TNA_BRANCH_CODE='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                    ...
                    sqlText += " DELETE from Sales_Inv_Detl" +
                                " WHERE sid_company_code='" + cbCompany.SelectedValue.ToString() +
                                "' AND sid_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                    ...
                    sqlText += " UPDATE Sales_Inv_Head set sih_invoice_date='" + Convert.ToDateTime(dtpInvoiceDate.Value).ToString("dd/MMM/yyy") +
                            ...
                            "  AND sih_branch_code='" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                            "' AND sih_fin_year='" + CommonData.FinancialYear.ToString() +
                            "' AND sih_company_code='" + cbCompany.SelectedValue.ToString() + "'";
                ...
                for (int i = 0; i < gvProductDetails.Rows.Count; i++)
                {
                    if (gvProductDetails.Rows[i].Cells["Rate"].Value.ToString() != "" && gvProductDetails.Rows[i].Cells["Qty"].Value.ToString() != "")
                    {
                        sqlText += " INSERT INTO SALES_INV_BULTIN_DETL(sibd_company_code, sibd_state_code, sibd_branch_code" +
                                    ...
                                    ", sibd_price, sibd_qty, sibd_amount, SIBD_PRODUCT_POINTS, SIBD_UNIT_POINTS,SIBD_BATCH_NO)" +
                                    " VALUES('" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                    "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] + "', " + txtInvoiceNo.Text + 
                                    ", '" + CommonData.FinancialYear + "'," + gvProductDetails.Rows[i].Cells["SLNO"].Value.ToString() +
                                    ", '" + cbBranch.SelectedValue.ToString().Split('@')[2] + 
                                    ...
                                    ",'" + gvProductDetails.Rows[i].Cells["BatchNo"].Value.ToString().ToUpper() + "'); ";

                        sqlText += " INSERT INTO Sales_Inv_Detl(sid_company_code, sid_state_code, sid_branch_code " +
                                     ", sid_invoice_number,sid_fin_year, sid_invoice_sl_no, sid_state, sid_product_id, sid_price " +
                                     ", sid_qty, sid_amount, SID_PRODUCT_POINTS, SID_UNIT_POINTS,SID_BATCH_NO)" +
                                    " VALUES ('" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                    "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] + "', " + txtInvoiceNo.Text +
                                    ", '" + CommonData.FinancialYear + "'," + gvProductDetails.Rows[i].Cells["SLNO"].Value +
                                    ", '" + cbBranch.SelectedValue.ToString().Split('@')[2] + "', '" + gvProductDetails.Rows[i].Cells["ProductID"].Value.ToString().Trim() +
                                    "', " + gvProductDetails.Rows[i].Cells["Rate"].Value + ", " + gvProductDetails.Rows[i].Cells["Qty"].Value +
                                    ", " + gvProductDetails.Rows[i].Cells["Amount"].Value + ", " + gvProductDetails.Rows[i].Cells["Points"].Value +
                                    "," + gvProductDetails.Rows[i].Cells["DBPoints"].Value +
                                    ",'" + gvProductDetails.Rows[i].Cells["BatchNo"].Value.ToString().ToUpper() + "'); ";
                    }
                ...
                for (int i = 0; i < dgvFreeProduct.Rows.Count; i++)
                {
                    ...
                    sqlText += " INSERT INTO Sales_Inv_Detl_FREE(sidf_company_code, sidf_state_code, sidf_branch_code, " +
                                ...
                                "sidf_given_date1, SIDF_SL_NO,SIDF_BATCH_NO)" +
                                " VALUES ('" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] + "', " + txtInvoiceNo.Text +
                                ...
                                ",'"+ dgvFreeProduct.Rows[i].Cells["FreeBatchNo"].Value.ToString().ToUpper() +"') ";

                    //}

                }

                sqlText += " exec SSCRM_SERVICE_TNA_AUD '" + cbCompany.SelectedValue.ToString() +
                                "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] +
                                "', '" + txtDocMonth.Text.ToUpper() +
                                "', '" + txtOrderNo.Text +
                                "', " + txtInvoiceNo.Text;
            ...
            try
            {
                ...
                for (int i = 0; i < gvProductDetails.Rows.Count; i++)
                {
                    if (gvProductDetails.Rows[i].Cells["Rate"].Value.ToString() != "" && gvProductDetails.Rows[i].Cells["Qty"].Value.ToString() != "")
                    {
                        ...
                        sqlText += " INSERT INTO Sales_Inv_Detl(sid_company_code, sid_state_code, sid_branch_code, sid_invoice_number "+
                                   ",sid_fin_year, sid_invoice_sl_no, sid_state, sid_product_id, sid_price, sid_qty, sid_amount "+
                                   ", SID_PRODUCT_POINTS, SID_UNIT_POINTS,SID_BATCH_NO)" +
                                    " VALUES ('" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                                    "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] + "', " + txtInvoiceNo.Text + 
                                    ", '" + CommonData.FinancialYear + "'," + gvProductDetails.Rows[i].Cells["SLNO"].Value +
                                    ", '" + cbBranch.SelectedValue.ToString().Split('@')[2] +
        ...
        private int SaveBulletInvoiceData()
        {
            ...
            try
            {
                ...
                sqlText += " INSERT INTO SALES_INV_BULTIN_HEAD(sibh_company_code, sibh_state_code, sibh_branch_code "+
                            ...
                            ", SIBH_INVOICE_AMOUNT, SIBH_ADVANCE_AMOUNT, SIBH_RECEIVED_AMOUNT, SIBH_CREDIT_SALE)" +
                             " SELECT '" + cbCompany.SelectedValue.ToString() + "', '" + cbBranch.SelectedValue.ToString().Split('@')[1] +
                             "', '" + cbBranch.SelectedValue.ToString().Split('@')[0] +"' , '" + CommonData.FinancialYear + 
                             "'," + txtInvoiceNo.Text + ", '" + Convert.ToDateTime(dtpInvoiceDate.Value).ToString("dd/MMM/yyy") + "' " +
                             ", '99999', '" + Convert.ToInt32(txtOrderNo.Text).ToString("00000") + "', '" + strECode + 
        ...
        {
            string sCompCode = "", sBranCode = "", sStateCode = "";
            //int nColIndex = 0;
            //if (strCompCode.Length > 2)
            //    nColIndex = 5;
            //else
            //    nColIndex = e.ColumnIndex;
            objSPDB = new StockPointDB();

            sCompCode = cbCompany.SelectedValue.ToString();
            sBranCode = cbBranch.SelectedValue.ToString().Split('@')[0];
            sStateCode = cbBranch.SelectedValue.ToString().Split('@')[1];

            if (e.ColumnIndex == 5)
            {
                if (Convert.ToString(gvProductDetails.Rows[e.RowIndex].Cells["Qty"].Value) != "")
                {
                    string[] strRatePoints = objSPDB.getProductRatePoints_ForSPInv(sCompCode, sBranCode, sStateCode, gvProductDetails.Rows[e.RowIndex].Cells["ProductId"].Value.ToString(), Convert.ToDouble(gvProductDetails.Rows[e.RowIndex].Cells["Qty"].Value.ToString()), Convert.ToDateTime(dtpInvoiceDate.Value.ToString("dd/MM/yyyy"))).Split('@');
                    if (strRatePoints[0] != "")
                    {
        ...
        //                        dgvFreeProduct.Rows[intCurRow].Cells["FreeBatchNo"].Value = "";
        ...
        private void FillFreeProductToGrid(string sProdCat, string sProdId, string sMainProd, double intQty, Int32 intInvDetlSlno)
        {
            ...
            if (isItemExisted == false)
            {
                ...
                dt = objSPDB.InvFreeProductForSPInv_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], cbBranch.SelectedValue.ToString().Split('@')[1], sProdId, intQty);
                if (dt.Rows.Count > 0)
                {
                ...
                while (remainder.ToString() != "0")
                {
                    ...
                    dt = objSPDB.InvFreeProductForSPInv_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], cbBranch.SelectedValue.ToString().Split('@')[1], sProdId, remainder);
                    if (dt.Rows.Count > 0)
                    {
        ...
        private void TotalMainProducts()
        {
            ...
            for (int nRow = 0; nRow < gvProductDetails.Rows.Count; nRow++)
            {
                ...
                if (Convert.ToString(gvProductDetails.Rows[nRow].Cells["Qty"].Value) != "")
                {
        ...
        private void FillInvoiceFreeProduct()
        {
            ...
            objSPDB = new StockPointDB();
            DataTable dt = new DataTable();
            dt = objSPDB.Get_InvoiceDetailFreeItemsForSPInvoice(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], Convert.ToInt32(txtInvoiceNo.Text));
            if (dt.Rows.Count == 0)
                dt = objSPDB.InvoiceBltnDetailFreeItemsForSPInvoice_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], Convert.ToInt32(txtInvoiceNo.Text));
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    dgvFreeProduct.Rows[intCurRow].Cells["FreeBatchNo"].Value = dr["BatchNo"] + "";
        ...
        private void txtInvoiceNo_Validated(object sender, EventArgs e)
        {
            ...
            //else
            //{
            //    MessageBox.Show("Please Select Company and Branch","SP-Invoice",MessageBoxButtons.OK,MessageBoxIcon.Information);
            //}
        }
        ...
        private bool CheckOrderInvoiceExist(int nInvNo, string sOrderNo)
        {
            ...
            try
            {
                ...
                objData = new InvoiceDB();
                ht = objSPDB.GetInvoiceDataForSPInvoice(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], nInvNo, sOrderNo);
                ...
                if (InvH.Rows.Count == 0)
                {
                    ...
                    ht = objSPDB.GetInvoiceBulitin_For_SPInvoice(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], nInvNo, sOrderNo);
                    InvH = (DataTable)ht["InvHead"];
                    InvD = (DataTable)ht["InvDetail"];
                }
                if (InvH.Rows.Count == 0)
        ...
        private void FillInvOrBultInData(int nInvNo, string sOrderNo)
        {
            ...
            DataTable InvH;
            DataTable InvD;
            DataTable dtSPInv = new DataTable();
            DataSet ds = new DataSet();
            objSPDB = new StockPointDB();
            try
            {
                ...
                if (InvH.Rows.Count == 0)
                {
                    ...
                    if (InvH.Rows.Count == 0)
                    {
                        ds = objSPDB.SalesOrderForSPInvoice_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], strFinYear, txtOrderNo.Text.ToString().Trim());
                        InvH = ds.Tables[0];
                        InvD = ds.Tables[1];
                        if (InvH.Rows.Count == 0)
                        { IsSaleOrder = false; }
                ...
                dtSPInv = objSPDB.Get_SPInvoiceHeadDetails(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], "", 0, Convert.ToInt32(txtInvoiceNo.Text), Convert.ToInt32(txtOrderNo.Text).ToString("00000")).Tables[0];
                if (dtSPInv.Rows.Count > 0)
                {
                    FillSPInvoiceDetails(dtSPInv);
                }
                else
                {
                    txtSpInvoiceNo.Text = GenerateNewStockPointInvNo().ToString();
        ...
        private void FillSPInvoiceDetails(DataTable dtSpInv)
        {
            try
            {
                if (dtSpInv.Rows.Count > 0)
                {
                    txtSpInvoiceNo.Text = dtSpInv.Rows[0]["SPInvNo"].ToString();
                    sSPDocMonth = dtSpInv.Rows[0]["SPDocMonth"].ToString();
                }
                else
                {
                    txtSpInvoiceNo.Text = GenerateNewStockPointInvNo().ToString();
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (Convert.ToDouble(dt.Rows[i]["amount"]) > 0)
                    {
                        ...
                        cellBatchNo.Value = dt.Rows[i]["BatchNo"].ToString();
            ...
            intCurrentCell = (((System.Windows.Forms.DataGridView)(sender))).CurrentCell.ColumnIndex;
             if ((((System.Windows.Forms.DataGridView)(sender))).CurrentCell.ColumnIndex == 5)
                ...
                if ((((System.Windows.Forms.DataGridView)(sender))).CurrentCell.ColumnIndex == 6)
                {
                    ...
                    //if (txtBatchNo != null)
                    //{
                    //    txtBatchNo.MaxLength = 28;
                    //    txtBatchNo.KeyPress += new KeyPressEventHandler(txt_KeyPress);
                    //}
                }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtInvoiceNo.Text.Length > 0)
                {
                    ...
                    {
                        DialogResult result = MessageBox.Show("Do you want to Delete " + txtSpInvoiceNo.Text + " invoice?",
                                               "CRM", MessageBoxButtons.YesNo);
                        if (result == DialogResult.Yes)
                        {
                            objSQLDB = new SQLDB();
                            string strDelete = " DELETE FROM SERVICES_TNA WHERE TNA_COMPANY_CODE='" + cbCompany.SelectedValue.ToString() + "'" +
                                              " AND TNA_BRANCH_CODE='" + cbBranch.SelectedValue.ToString().Split('@')[0] + "'" +
                                              " AND TNA_INVOICE_NUMBER=" + txtInvoiceNo.Text +
                                              " AND TNA_FIN_YEAR='" + CommonData.FinancialYear + "'";
                            ...
                            //{
                                ...
                                strDelete += " DELETE FROM SALES_INV_SP_HEAD "+
                                            " where SISH_BRANCH_CODE='"+ cbBranch.SelectedValue.ToString().Split('@')[0]+
                                            "' and SISH_INVOICE_NO="+ txtInvoiceNo.Text+
                                            " and SISH_FIN_YEAR='"+ CommonData.FinancialYear +
                                            "'and SISH_SP_BRANCH_CODE='" + CommonData.BranchCode +
                                            "' and SISH_SP_SP_INVOICE_NO=" + txtSpInvoiceNo.Text + "";
            ...
            foreach (System.Windows.Forms.Control ctrControl in parent.Controls)
            {
                ...
                if (object.ReferenceEquals(ctrControl.GetType(), typeof(System.Windows.Forms.TextBox)))
                {
                    ...
                    if ((((System.Windows.Forms.TextBox)ctrControl).Name != "txtEcodeSearch") &&
                        ...
                        (((System.Windows.Forms.TextBox)ctrControl).Name != "txtSpInvoiceNo"))
                    {
        ...
        {
            VillageSearch VSearch = new VillageSearch("SPInvoice");
            VSearch.objFrmSPInvoice = this;
            VSearch.ShowDialog();
        }
        ...
        private void cbEcode_KeyPress(object sender, KeyPressEventArgs e)
       {
        ...
        private void btnProductSearch_Click(object sender, EventArgs e)
        {
            ...
            if (cbCompany.SelectedIndex > 0 && cbBranch.SelectedIndex > 0)
            {
                ...
                ProductSearchAll PSearch = new ProductSearchAll("SPInvoice", cbCompany.SelectedValue.ToString(),sBranCode,sStateCode);
                PSearch.objFrmSPInvoice = this;
                PSearch.ShowDialog();
            }
            else
            {
                MessageBox.Show("Please Select Company and Branch","SSERP",MessageBoxButtons.OK,MessageBoxIcon.Information);
            }
        }
        ...
        private void EcodeSearch()
        {
            ...
            if (cbCompany.SelectedIndex > 0 && cbBranch.SelectedIndex > 0)
            {
                try
                {
                    ...
                    cbEcode.Items.Clear();
                    dsEmp = objData.InvLevelEcodeSearch_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], txtDocMonth.Text, txtEcodeSearch.Text.ToString());
                    DataTable dtEmp = dsEmp.Tables[0];
                    if (dtEmp.Rows.Count > 0)
                    {
                        cbEcode.DataSource = dtEmp;
        ...
        private void btnAddFreeProduct_Click(object sender, EventArgs e)
        {
            if (gvProductDetails.Rows.Count > 0)
            {
                AddFreeProducts freeProduct = new AddFreeProducts(gvProductDetails, "SP_INVOICE",cbCompany.SelectedValue.ToString());
                freeProduct.objSPInvoice = this;
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (dgvFreeProduct.Rows[e.RowIndex].Cells["Edit"].Value.ToString().Trim() != "")
                {
                    if (Convert.ToBoolean(dgvFreeProduct.Rows[e.RowIndex].Cells["Edit"].Selected) == true)
                    {
                        ...
                        AddFreeProducts oAddFreeProducts = new AddFreeProducts(e.RowIndex, dgvr, "SP_INVOICE",cbCompany.SelectedValue.ToString());
                        oAddFreeProducts.objSPInvoice = this;
        ...
        private void FSEcodeSearch()
        {
            if (txtFSEcode.Text.Length > 0)
            {
                ...
                try
                {
                    ...
                    dsEmp = objData.InvLevelFSEcodeSearch_Get(cbCompany.SelectedValue.ToString(), cbBranch.SelectedValue.ToString().Split('@')[0], txtDocMonth.Text, txtFSEcode.Text.ToString(), Convert.ToInt32(strECode));
                    DataTable dtEmp = dsEmp.Tables[0];
                    if (dtEmp.Rows.Count > 0)
                    {
        ...
        private void txtSpInvoiceNo_Validated(object sender, EventArgs e)
        {
            objSPDB = new StockPointDB();
            DataTable dt = new DataTable();
            ClearForm(this);
            if (Convert.ToString(txtSpInvoiceNo.Text).Trim().Length > 0)
            {
                try
                {
                    dt = objSPDB.Get_SPInvoiceHeadDetails("", "", CommonData.BranchCode, Convert.ToInt32(txtSpInvoiceNo.Text), 0, "").Tables[0];
                    if (dt.Rows.Count > 0)
                    {                        
        ...
        private void txtSpInvoiceNo_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != '\b')
            {
        ...
        private bool CheckOrderNoExistInSPInvoice(Int32 nInvNo,string strOrderNo)
        {
            objSPDB = new StockPointDB();
            DataTable dt = new DataTable();
            string sCompCode = "";
            if (cbCompany.SelectedIndex > 0 && cbBranch.SelectedIndex > 0)
            {
                try
                {
                    ...
                    dt = objSPDB.Get_SPInvoiceHeadDetails(sCompCode, cbBranch.SelectedValue.ToString().Split('@')[0],"",0, nInvNo, strOrderNo).Tables[0];
                    if (dt.Rows.Count == 0)
                    {
                        isSPInvExist = false;
                    }
