...
namespace SSCRM
{
    public partial class StockPointGRN : Form
    {
        ...
        private void DeliveryChallanPU_Load(object sender, EventArgs e)
        {
            ...
            gvGRNDetails.ColumnHeadersDefaultCellStyle.Font = new System.Drawing.Font("Microsoft Sans Serif", 9,
        ...
        private void FillTransactionType()
        {
            ...
            try
            {
                DataTable dt = objUtilData.dtSPGRNTranType();
                if (dt.Rows.Count > 0)
                {
                    TranTypeLoad = true;
        ...
        private void CalculateTotals()
        {
            ...
            if (gvGRNDetails.Rows.Count > 0)
            {
                ...
                {
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["RecdQty"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["RecdQty"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["DamageQty"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["DamageQty"].Value = 0; }
                    try { gvGRNDetails.Rows[i].Cells["TotalQty"].Value = (Convert.ToDouble(gvGRNDetails.Rows[i].Cells["RecdQty"].Value.ToString()) + 
                                                                        Convert.ToDouble(gvGRNDetails.Rows[i].Cells["DamageQty"].Value.ToString())).ToString("f"); }
                    catch { gvGRNDetails.Rows[i].Cells["TotalQty"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["Rate"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["Rate"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["Amount"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["Amount"].Value = 0; }
               ...
               // FillTransactionPUHeadData(strDCNo);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int intRow = 0; intRow < dt.Rows.Count; intRow++)
                    {
                        ...
                        try
                        {
                            gvGRNDetails.Rows[intRow].Cells["RecdQty"].Value = (Convert.ToDouble(dt.Rows[intRow]["RecdQty"].ToString()) - Convert.ToDouble(dt.Rows[intRow]["DmgQty"].ToString())).ToString("f");
                        }
                        catch
                        {
                            gvGRNDetails.Rows[intRow].Cells["RecdQty"].Value = 0;
                        }
                        //else
                        gvGRNDetails.Rows[intRow].Cells["DamageQty"].Value = dt.Rows[intRow]["DmgQty"].ToString();
                        gvGRNDetails.Rows[intRow].Cells["TotalQty"].Value = dt.Rows[intRow]["RecdQty"].ToString();
        ...
        private void cbBranches_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (txtEcodeSearch.Text.ToString().Length > 2)
        ...
        private void FillBranchGroupData(Int32 ecode)
        {
            ...
            try
            {
                ...
                if (ecode == 0 || dtGroup.Rows.Count==0)
                {
            ...
            {
                MessageBox.Show(ex.Message, "GRN SP", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            finally
            {
                objIndent = null;
                ...
                try
                {
                    if (chkCancelSPGRN.Checked == true)
                    {
                        if (IsModify == false)
                        {
                            ...
                            strCommand = " INSERT INTO SP_GRN_HEAD " +
                                 "(SPGH_COMPANY_CODE" +
                                 ", SPGH_STATE_CODE" +
                                 ", SPGH_BRANCH_CODE" +
                                 ", SPGH_FIN_YEAR" +
                        ...
                        {
                            strCommand = " DELETE from SP_GRN_DETL" +
                            ...
                            strCommand += " DELETE from SP_GRN_PKM_DETL" +
                                    " WHERE SPGPD_company_code='" + CommonData.CompanyCode +
                                        "' AND SPGPD_BRANCH_CODE='" + CommonData.BranchCode +
                                        "' AND SPGPD_GRN_NUMBER='" + txtTransactionNo.Text +
                                        "' AND SPGPD_FIN_YEAR='" + sFinYear + "'";

                            int intRec = objSQLDB.ExecuteSaveData(strCommand);

                            strCommand = " UPDATE SP_GRN_HEAD " +
                                         ...
                                         "', SPGH_REMARKS='" + txtRemarks.Text.ToString() +
                                         "', SPGH_GRN_TYPE='" + cbTransactionType.Text +
                                         "', SPGH_STATUS='CANCELLED" +
                                         "', SPGH_STATE_CODE='" + CommonData.StateCode +
                                         "', SPGH_DOCUMENT_MONTH='" + txtDocMonth.Text.ToUpper() +
                    ...
                    else
                    {
                        ...
                        if (SaveDCHeadData() > 0)
                        {
                            ...
                            //if (intSave > 0)
                            //{
                            if (dgvPKMDetls.Rows.Count > 0)
                                intSave = SaveDCPKMDetlData();
                            else
                                intSave++;
                            ...
                            //else
                            //{
                                //string strSQL = " DELETE from SP_GRN_DETL" +
                                //                    " WHERE SPGD_company_code='" + CommonData.CompanyCode +
                                //                    "' AND SPGD_BRANCH_CODE='" + CommonData.BranchCode +
                                //                    "' AND SPGD_GRN_NUMBER='" + txtTransactionNo.Text +
                                //                    "' AND SPGD_FIN_YEAR='" + sFinYear + "'";

                                //strSQL += " DELETE from SP_GRN_PKM_DETL" +
                                //        " WHERE SPGPD_company_code='" + CommonData.CompanyCode +
                                //            "' AND SPGPD_BRANCH_CODE='" + CommonData.BranchCode +
                                //            "' AND SPGPD_GRN_NUMBER='" + txtTransactionNo.Text +
                                //            "' AND SPGPD_FIN_YEAR='" + sFinYear + "'";

                                //strSQL += " DELETE from SP_GRN_HEAD" +
        ...
        private int SaveDCPKMDetlData()
        {
            ...
            try
            {
                ...
                int iSlNO = 0;
                for (int i = 0; i < dgvPKMDetls.Rows.Count; i++)
                {
                    try { Convert.ToDouble(dgvPKMDetls.Rows[i].Cells["pkTotalQty"].Value.ToString()); }
                    catch { dgvPKMDetls.Rows[i].Cells["pkTotalQty"].Value = 0; }
                    try { Convert.ToDouble(dgvPKMDetls.Rows[i].Cells["pkDCQty"].Value.ToString()); }
                    catch { dgvPKMDetls.Rows[i].Cells["pkDCQty"].Value = 0; }
                    try { Convert.ToDouble(dgvPKMDetls.Rows[i].Cells["pkRate"].Value.ToString()); }
                    catch { dgvPKMDetls.Rows[i].Cells["pkRate"].Value = 0; }
                    try { Convert.ToDouble(dgvPKMDetls.Rows[i].Cells["pkAmount"].Value.ToString()); }
                    catch { dgvPKMDetls.Rows[i].Cells["pkAmount"].Value = 0; }
                    try { Convert.ToDouble(dgvPKMDetls.Rows[i].Cells["pkDamageQty"].Value.ToString()); }
                    catch { dgvPKMDetls.Rows[i].Cells["pkDamageQty"].Value = 0; }

                    if (dgvPKMDetls.Rows[i].Cells["pkTotalQty"].Value.ToString() != "" &&
                        dgvPKMDetls.Rows[i].Cells["pkTotalQty"].Value.ToString() != "0" &&
                        dgvPKMDetls.Rows[i].Cells["pkAmount"].Value.ToString() != "")
                    {
                        iSlNO++;
                        sbSQL.Append(" INSERT INTO SP_GRN_PKM_DETL (SPGPD_COMPANY_CODE, SPGPD_STATE_CODE, SPGPD_BRANCH_CODE"+
                                    ", SPGPD_FIN_YEAR, SPGPD_DOCUMENT_MONTH, SPGPD_GRN_TYPE, SPGD_GRPN_NUMBER, SPGPD_SL_NO"+
                                    ", SPGPD_PRODUCT_ID, SPGPD_BATCH_NO, SPGPD_GRN_QTY, SPGPD_REC_RATE, " +
                                    "SPGPD_TOT_QTY, SPGPD_GOOD_QTY, SPGPD_DAMAGE_QTY, SPGPD_REC_AMT, SPGPD_AGNST_TRN_TYPE, SPGPD_AGNST_TRN_NUMBER)" +
                                    " VALUES ('" + CommonData.CompanyCode + "', '" + CommonData.StateCode +
                                    "', '" + CommonData.BranchCode + "', '" + sFinYear + "', '" + txtDocMonth.Text +
                                    "', '" + cbTransactionType.Text + "', '" + txtTransactionNo.Text +
                                    "', " + iSlNO +
                                    ", '" + gvGRNDetails.Rows[i].Cells["pkProductId"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkBatchNo"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkDCQty"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkRate"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkTotalQty"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkRecdQty"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkDamageQty"].Value +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkAmount"].Value +
                                    "', '" + cbTransactionType.Text +
                                    "', '" + gvGRNDetails.Rows[i].Cells["pkDCNo"].Value + "'); ");
        ...
        private int SaveDCHeadData()
        {
            ...
            try
            {
                ...
                if (cbTransactionType.SelectedValue.ToString() != "BR2SP" && cbTransactionType.SelectedValue.ToString() != "GL2SP")
                    ecode = "0";
                else
                ...
                {
                    txtTransactionNo.Text = NewTransactionNumber().ToString();
                    if (txtTransactionNo.Text.Replace(" ", "").Length > 0)
                    {
                ...
                else
                {
                    if (txtTransactionNo.Text.Replace(" ", "").Length > 0)
                    {
                        ...
                        strSQL = " UPDATE SP_GRN_HEAD " +
                                    ...
                                    "', SPGH_STATUS='DOCUMENTED" +
                                    "', SPGH_STATE_CODE='" + CommonData.StateCode +
                                    "', SPGH_DOCUMENT_MONTH='" + txtDocMonth.Text.ToUpper() +
                                    "', SPGH_REMARKS='" + txtRemarks.Text.ToString().Replace("'", "") +
                                    "',SPGH_FROM_BRANCH_CODE='" + cbBranches1.SelectedValue.ToString() +
        ...
        private int SaveDCDetlData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gvGRNDetails.Rows.Count; i++)
                {
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["TotalQty"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["TotalQty"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["DCQty"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["DCQty"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["Rate"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["Rate"].Value = 0; }
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["Amount"].Value.ToString()); }
                    catch { gvGRNDetails.Rows[i].Cells["Amount"].Value = 0; }
                    ...
                    if (gvGRNDetails.Rows[i].Cells["TotalQty"].Value.ToString() != "" &&
                        gvGRNDetails.Rows[i].Cells["TotalQty"].Value.ToString() != "0" && 
                        gvGRNDetails.Rows[i].Cells["Amount"].Value.ToString() != "")
                    {
                        iSlNO++;
                        sbSQL.Append(" INSERT INTO SP_GRN_DETL (SPGD_COMPANY_CODE, SPGD_STATE_CODE, SPGD_BRANCH_CODE, SPGD_FIN_YEAR, SPGD_DOCUMENT_MONTH, SPGD_GRN_TYPE, SPGD_GRN_NUMBER, SPGD_SL_NO, SPGD_PRODUCT_ID, SPGD_BATCH_NO, SPGD_GRN_QTY, SPGD_REC_RATE, " +
                                    ...
                                    "', '" + cbTransactionType.Text + 
                                    "', '" + gvGRNDetails.Rows[i].Cells["DCNo"].Value + "'); ");
        ...
        private bool CheckData()
        {
            ...
            if (chkCancelSPGRN.Checked == true)
            {
                ...
                {
                    MessageBox.Show("Please Enter Remarks", "SP GRN", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    blValue = false;
                    txtRemarks.Focus();
                    return blValue;
                }
            ...
            else if (chkCancelSPGRN.Checked == false)
            {
                ...
                if (cbTransactionType.SelectedValue == "BR2SP")
                {
                    if (cbEcode.SelectedValue == "0")
                    {
                        MessageBox.Show("Invalid E-Code!", "SP GRN", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        blValue = false;
                ...
                for (int i = 0; i < gvGRNDetails.Rows.Count; i++)
                {
                    try { Convert.ToDouble(gvGRNDetails.Rows[i].Cells["TotalQty"].Value); }
                    catch { gvGRNDetails.Rows[i].Cells["TotalQty"].Value = 0; }

                    if (Convert.ToDouble(gvGRNDetails.Rows[i].Cells["TotalQty"].Value) != 0)
                    {
                        blInvDtl = true;
                    }
                }
        ...
        private void FillTransactionData(string TranNO)
        {
            ...
            try
            {
                ...
                Rs = objPUDB.GRN_StockPoint("", "", TranNO);
        ...
        private bool FillTransactionHead(DataTable dtHead, DataTable dtDetl)
        {
            ...
            try
            {
                if (dtHead.Rows.Count > 0)
                {
                    ...
                    if (dtHead.Rows[0]["FromBranchCode"].ToString() != "")
                    {
        ...
        private void OrderSlNo()
        {
            ...
            if (dgvPKMDetls.Rows.Count > 0)
            {
                for (int i = 0; i < dgvPKMDetls.Rows.Count; i++)
                {
                    dgvPKMDetls.Rows[i].Cells["pkSlNo"].Value = i + 1;
                }
            }
        }
        ...
        private void cbTransactionType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbTransactionType.SelectedIndex > -1 && TranTypeLoad == false)
            {
                ...
                if(cbBranches1.Items.Count!=0)
                    cbBranches1.SelectedIndex = 0;
                if (cbTransactionType.SelectedValue.ToString() == "PU2SP")
                {
                    cbEcode.Enabled = false;
                    txtEcodeSearch.Enabled = false;
                }
                else if (cbTransactionType.SelectedValue.ToString() == "BR2SP")
                {
        ...
        private void EcodeSearch()
        {
            ...
            try
            {
                ...
                cbEcode.DataSource = null;
                cbEcode.Items.Clear();
                dsEmp = objInvoiceDB.InvLevelGLEcodeSearch_Get(CommonData.CompanyCode, cbBranches1.SelectedValue.ToString(), CommonData.DocMonth, txtEcodeSearch.Text.ToString());
        ...
        public void FillEmployeeData()
        {
            ...
            try
            {
                ...
                dsEmp = objInvoiceDB.GetGLEcodeList(CommonData.CompanyCode, cbBranches1.SelectedValue.ToString(), CommonData.DocMonth);
        ...
        private void FillGRNDetails(string grnRefNo)
        {
            ...
            try
            {
                sGrnInfo = objStockPointDB.SPGRNHeadDetails_Get(CommonData.CompanyCode, CommonData.BranchCode, CommonData.DocMonth, txtReferenceNo.Text.ToString());
        ...
        private void FillGRNDetlDatatoGrid(string sGrnNo)
        {
            ...
            sGrnDetl = objStockPointDB.SPGRNDetlInfo_Get(CommonData.CompanyCode, CommonData.BranchCode, CommonData.DocMonth, txtTransactionNo.Text.ToString()).Tables[0];
            gvGRNDetails.Rows.Clear();
        ...
        private void btnPrintEmpList_Click(object sender, EventArgs e)
        {
            ...
            childReportViewer = new ReportViewer("", cbBranches1.SelectedValue.ToString(), "", txtDocMonth.Text.ToUpper(), "");
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == 5 || e.ColumnIndex == 6)
                {
                    ...
                    if (textBoxCell != null)
                    {
                        dgvPKMDetls.CurrentCell = textBoxCell;
                        dgv.BeginEdit(true);
                    }
                }
                if (e.ColumnIndex == dgvPKMDetls.Columns["pkDelete"].Index)
                {
                    DialogResult dlgResult = MessageBox.Show("Do you want delete this record?", "Confirm?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (dlgResult == DialogResult.Yes)
                    {
                        DataGridViewRow dgvr = dgvPKMDetls.Rows[e.RowIndex];
                        dgvPKMDetls.Rows.Remove(dgvr);
                        OrderSlNo();
                    }
                }
            }
        ...
        private void dgvPKMDetls_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 6 && e.ColumnIndex <= 18)
            {
                if (Convert.ToString(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRecdQty"].Value) != "")
                {
                    try { Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRecdQty"].Value); }
                    catch { dgvPKMDetls.Rows[e.RowIndex].Cells["pkRecdQty"].Value = 0; }
                    try { Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRate"].Value); }
                    catch { dgvPKMDetls.Rows[e.RowIndex].Cells["pkRate"].Value = 0; }

                    if (Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRecdQty"].Value) >= 0 && Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRate"].Value) >= 0)
                    {
                        dgvPKMDetls.Rows[e.RowIndex].Cells["pkAmount"].Value = (Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRecdQty"].Value) * Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkRate"].Value));
                        dgvPKMDetls.Rows[e.RowIndex].Cells["pkAmount"].Value = Convert.ToDouble(dgvPKMDetls.Rows[e.RowIndex].Cells["pkAmount"].Value).ToString("f");
                    }

                }
                else
                {
                    dgvPKMDetls.Rows[e.RowIndex].Cells["pkAmount"].Value = string.Empty;
