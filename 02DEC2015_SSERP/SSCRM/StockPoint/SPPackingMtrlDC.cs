...
namespace SSCRM
{
    public partial class SPPackingMtrlDC : Form
    {
        ...
        private string NewTransactionNumber()
        {
            ...
            try
            {
                sTranNo = objIndent.GenerateNewDCTranNo(CommonData.CompanyCode, CommonData.BranchCode, cbTransactionType.SelectedValue.ToString());
            }
            catch (Exception ex)
            {
        ...
        private int SaveDCDetlData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gvIndentDetails.Rows.Count; i++)
                {
                    if (gvIndentDetails.Rows[i].Cells["TotalQty"].Value.ToString() != "" && gvIndentDetails.Rows[i].Cells["TotalQty"].Value.ToString() != "0" && gvIndentDetails.Rows[i].Cells["Amount"].Value.ToString() != "")
                    {
                        sbSQL.Append("INSERT INTO SP_DC_DETL (SPDD_COMPANY_CODE, SPDD_STATE_CODE, SPDD_BRANCH_CODE, SPDD_FIN_YEAR, SPDD_DOCUMENT_MONTH, SPDD_TRN_TYPE, SPDD_TRN_NUMBER, SPDD_GRNISS_NUMBER, SPDD_SL_NO, SPDD_PRODUCT_ID, SPDD_ISS_QTY, SPDD_ISS_DAMG_QTY, SPDD_ISS_RATE, SPDD_ISS_AMT, SPDD_INDENT_NUMBER, SPDD_INDENT_SLNO)" +
                                    ...
                                    ", '" + gvIndentDetails.Rows[i].Cells["ProductId"].Value + "', " + gvIndentDetails.Rows[i].Cells["TotalQty"].Value + ", " + gvIndentDetails.Rows[i].Cells["DmgQty"].Value + 
                                    ", " + gvIndentDetails.Rows[i].Cells["Rate"].Value + ", " + gvIndentDetails.Rows[i].Cells["Amount"].Value +
                                    ", " + gvIndentDetails.Rows[i].Cells["IndentNo"].Value + ", " + gvIndentDetails.Rows[i].Cells["IndentSLNo"].Value + "); ");
        ...
        private void gvProductDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 5  && e.ColumnIndex <= 8)
            {
                ...
                //{
                    ...
                    {
                        gvIndentDetails.Rows[e.RowIndex].Cells["Amount"].Value = tQty * (Convert.ToDouble(gvIndentDetails.Rows[e.RowIndex].Cells["Rate"].Value));
