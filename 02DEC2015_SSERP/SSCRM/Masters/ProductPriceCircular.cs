...
namespace SSCRM
{
    public partial class ProductPriceCircular : Form
    {
        SQLDB objSQLData = null;
        bool flagUpdate = false;
        
       
        public ProductPriceCircular()
        {
            InitializeComponent();
        }

        private void ProductPriceCircular_Load(object sender, EventArgs e)
        {
            FillCompanyData();
        ...
        private void FillBranchData()
        {
            ...
            try
            {
                if (cbCompany.SelectedIndex > 0)
                {
                    ...
                    string strCommand = "SELECT BRANCH_NAME,BRANCH_CODE+'@'+ STATE_CODE AS val"+
                                        ",BRANCH_TYPE FROM BRANCH_MAS where COMPANY_CODE='" + cbCompany.SelectedValue.ToString() +
        ...
        {
            ProductSearchAll PSearch = new ProductSearchAll("ProductPriceCircular");
            PSearch.objProductPriceCircular = this;
            PSearch.ShowDialog();
            SetSubProdSlNo();
        }
        ...
        private void FillProductsToGrid()
        {
            ...
            if (cbUserBranch.SelectedIndex > 0)
            {
                try
                {
                    string[] strArr = cbUserBranch.SelectedValue.ToString().Split('@');
        ...
        private void FillSubProducts(string sProdID)
        {
            objSQLData=new SQLDB();
            DataTable dt=new DataTable();
            ...
            if (CheckData()== true)
            {
                ...
                try
                {
                    if (gvProductDetails.Rows.Count > 0)
                    {
                        ...
                        //strCommand = "DELETE FROM PRODUCT_PRICE_MASTER_LI WHERE   PPML_COMP_CODE='" + cbCompany.SelectedValue.ToString() + "' AND PPML_BRANCH_CODE='" + strArr[0] + "' AND PPML_WEF_DATE='" + Convert.ToDateTime(dtpEffDate.Value).ToString("dd/MMM/yyyy") + "' ";
                        //iResult1 = objSQLData.ExecuteSaveData(strCommand);


                        strCommand = " DELETE FROM PRODUCT_PRICE_MASTER WHERE PPM_COMP_CODE='" + cbCompany.SelectedValue.ToString() + "' AND PPM_BRANCH_CODE='" + strArr[0] + "' AND PPM_WEF_DATE='" + Convert.ToDateTime(dtpEffDate.Value).ToString("dd/MMM/yyyy") + "'";
                            iResult1 = objSQLData.ExecuteSaveData(strCommand);
        ...
        private int SaveSubProducts()
        {
            ...
            try
            {
                ...
                {
                    strInsert += " DELETE FROM PRODUCT_PRICE_MASTER_LI WHERE" +
                                        " PPML_COMP_CODE='" + cbCompany.SelectedValue.ToString() +
                                        "' AND PPML_BRANCH_CODE='" + strArr[0] +
                                        "' AND PPML_PRODUCT_ID='" + gvSubProdDetails.Rows[0].Cells["SProductId"].Value.ToString() +
        ...
        private bool CheckData()
        {
            ...
            {
                MessageBox.Show("Select Branch", "Product Price Circular", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cbUserBranch.Focus();
                flag = false;
            }
            ...
            if(e.RowIndex >-1)
            {
                if (e.ColumnIndex == gvProductDetails.Columns["Delete"].Index)
                {
                    ...
                    if (dlgResult == DialogResult.Yes)
                    {
                        ...
                        //    for (int i = 0; i < gvSubProdDetails.Rows.Count; i++)
                        //    {
                        //        if (gvSubProdDetails.Rows[i].Cells["SProductId"].Value.ToString() == productId)
                        //        {
                        ...
                        strInsert += " DELETE FROM PRODUCT_PRICE_MASTER_LI WHERE" +
                                        " PPML_COMP_CODE='" + cbCompany.SelectedValue.ToString() +
                                        "' AND PPML_BRANCH_CODE='" + strArr[0] +
                                        "' AND PPML_PRODUCT_ID='" + productId +
                                        "' AND PPML_WEF_DATE='" + Convert.ToDateTime(dtpEffDate.Value).ToString("dd/MMM/yyyy") + "' ";
                        strInsert += " DELETE FROM PRODUCT_PRICE_MASTER WHERE "+
                                        "PPM_COMP_CODE='" + cbCompany.SelectedValue.ToString() + 
                                        "' AND PPM_BRANCH_CODE='" + strArr[0] +
                                        "' AND PPM_PRODUCT_ID='" + productId + 
                                        "' AND PPM_WEF_DATE='" + Convert.ToDateTime(dtpEffDate.Value).ToString("dd/MMM/yyyy") + "'";
                        try
                        {
                            objSQLData.ExecuteSaveData(strInsert);
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show(ex.ToString());
                ...
                {
                    FillSubProducts(gvProductDetails.Rows[e.RowIndex].Cells["ProductId"].Value.ToString());
                }
            }
        ...
        {
            if (cbUserBranch.SelectedIndex > 0 && cbCompany.SelectedIndex > 0)
            {
                objSQLData = new SQLDB();                
                ...
                dtpEffDate_ValueChanged(null, null);
            }
        ...
        private void gvSubProdDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                if (e.ColumnIndex == gvSubProdDetails.Columns["singleVatPer"].Index
                    || e.ColumnIndex == gvSubProdDetails.Columns["subProdOffPrice"].Index
                    || e.ColumnIndex == gvSubProdDetails.Columns["singleSalePrice"].Index)
                {
                    try { Convert.ToDouble(gvSubProdDetails.Rows[e.RowIndex].Cells["singleVat"].Value.ToString()); }
                    catch { gvSubProdDetails.Rows[e.RowIndex].Cells["singleVat"].Value = 0; }
                    try { Convert.ToDouble(gvSubProdDetails.Rows[e.RowIndex].Cells["subProdOffPrice"].Value.ToString()); }
        ...
        private void gvSubProdDetails_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
