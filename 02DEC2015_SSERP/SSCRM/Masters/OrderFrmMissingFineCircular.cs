...
namespace SSCRM
{
    public partial class OrderFrmMissingFineCircular : Form
    {
        bool flagUpdate = false;
        string strCmpData = "";
        string strBranchData = "";
        SQLDB objDB = null;
        public OrderFrmMissingFineCircular()
        {
            InitializeComponent();
        }
        ...
        private void OrderFrmMissingFineCircular_Load(object sender, EventArgs e)
        {
            FillRoles();
            FillCompanyData();
            //FillDefaultGridData();
        ...
        private bool checkData()
        {
           ...
           {
               MessageBox.Show("Please Enter Fine Amount", "", MessageBoxButtons.OK, MessageBoxIcon.Stop);
               flag = false;
               return flag;
           }
        ...
        private int SaveCircularData()
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                    ...
                    strCMD += " insert into ORDER_FORMS_FINE_CIRCULARS(OFFC_COMP_CODE," +
                                                                        ...
                                                                        "' AND OFFC_BRANCH_CODE='" + str[i].Split('@')[0] + "' AND OFFC_DOC_TYPE='MISSING' AND OFFC_EFF_DATE='"
                                                                        + dtpFDate.Value.ToString("dd/MMM/yyyy") + "' and OFFC_ROLE='" + cbRole.SelectedValue + "' AND OFFC_SINO=0)";
        ...
        private void FillData()
        {
            if (strCmpData.Length > 0 && strBranchData.Length > 0)
            {
                ...
                try
                {
                    ...
                    param[4] = objDB.CreateParameter("@xDocType", DbType.String, "MISSING", ParameterDirection.Input);
