...
namespace SSCRM
{
    public partial class Login : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
           uint iInt = 312;
            ...
            if (chkHO.CheckState == CheckState.Checked)
                CommonData.ConnectionType = "LOCAL";
            else
                CommonData.ConnectionType = "";
        ...
        private bool CheckData()
        {
            ...
            {
                MessageBox.Show("Enter User Id!", "SS ERP");
                txtUserId.Focus();
                dBool = false;
                return dBool;
            }
            ...
            {
                MessageBox.Show("Enter Pasword!", "SS ERP");
                txtPassword.Focus();
                dBool = false;
                return dBool;
            }
            if (txtUserId.Text.ToUpper() != "ADMIN" && cbUserBranch.Items.Count == 0)
            {
                MessageBox.Show("No Branch Access to your ID", "SS ERP");
                //txtPassword.Focus();
                dBool = false;
                return dBool;
            }
        ...
        private void txtPassword_Validated(object sender, EventArgs e)
        {
            ...
            try
            {
                if (txtUserId.Text.ToString().Trim().Length > 2 || txtPassword.Text.ToString().Trim().Length > 2)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        CommonData.LogUserName = dt.Rows[0]["UserName"].ToString();
                        if (!txtUserId.Text.Contains("admin"))
                        {
                            FillUserBranch();
                        }
