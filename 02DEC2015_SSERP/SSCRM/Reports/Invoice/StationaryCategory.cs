...
namespace SSCRM
{
    public partial class StationaryCategory : Form
    {
        ...
        public StationaryCategory(int iForm)
        {
            iFrmType = iForm;
            InitializeComponent();
        }
        ...
        private void StationaryCategory_Load(object sender, EventArgs e)
        {
            ...
            FillBranches();
            FillStationaryItems();
            dtpFromDate.Value = Convert.ToDateTime(Convert.ToDateTime(CommonData.CurrentDate).ToString("dd/MMM/yyyy").ToUpper());
            dtpToDate.Value = Convert.ToDateTime(Convert.ToDateTime(CommonData.CurrentDate).ToString("dd/MMM/yyyy").ToUpper());
        ...
        private void FillBranches()
        {
            ...
            tvBranches.Nodes.Clear();
            TreeNode tNode;
            tNode = tvBranches.Nodes.Add("Company&Branches");
            try
            {
                ds = Get_UserBranchStateFilterCursor("", "", CommonData.LogUserId, cbLocationType.SelectedValue.ToString(), "PARENT");
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        dsState = Get_UserBranchStateFilterCursor(ds.Tables[0].Rows[i]["COMPANY_CODE"].ToString(), "", CommonData.LogUserId, cbLocationType.SelectedValue.ToString(), "STATE");
                        ...
                        if (dsState.Tables[0].Rows.Count > 0)
                        {
                            ...
                            {
                                tvBranches.Nodes[0].Nodes[i].Nodes.Add(dsState.Tables[0].Rows[j]["StateCode"].ToString(), dsState.Tables[0].Rows[j]["StateName"].ToString());

                                DataSet dschild = new DataSet();

                                dschild = Get_UserBranchStateFilterCursor(ds.Tables[0].Rows[i]["COMPANY_CODE"].ToString(), dsState.Tables[0].Rows[j]["StateCode"].ToString(), CommonData.LogUserId, cbLocationType.SelectedValue.ToString(), "CHILD");
                                ...
                                if (dschild.Tables[0].Rows.Count > 0)
                                {
                                    for (int k = 0; k < dschild.Tables[0].Rows.Count; k++)
                                    {
                                        ...
                                        tvBranches.Nodes[0].Nodes[i].Nodes[j].Nodes.Add(dschild.Tables[0].Rows[k]["BranCode"].ToString(), dschild.Tables[0].Rows[k]["BranchName"].ToString());
        ...
        private void FillBranchTypes()
        {
            ...
            string strCommand = "";
            cbLocationType.DataSource = null;
            try
            {
                strCommand = "SELECT Distinct(BRANCH_TYPE) BranchType " +
                             ...
                             "  FROM BRANCH_MAS WHERE BRANCH_TYPE NOT IN('HO','PO','ST') ORDER BY BranchTypeName";
                dt = objSQLdb.ExecuteDataSet(strCommand).Tables[0];

                if (dt.Rows.Count > 0)
                {
                    cbLocationType.DataSource = dt;
                    cbLocationType.DisplayMember = "BranchTypeName";
                    cbLocationType.ValueMember = "BranchType";
        ...
        private void GetSelectedCompAndBranches()
        {
            ...
            bool iscomp = false;
            bool iSstate = false;
            for (int i = 0; i < tvBranches.Nodes[0].Nodes.Count; i++)
            {
                for (int j = 0; j < tvBranches.Nodes[0].Nodes[i].Nodes.Count; j++)
                {
                    for (int k = 0; k < tvBranches.Nodes[0].Nodes[i].Nodes[j].Nodes.Count; k++)
                    {

                        if (tvBranches.Nodes[0].Nodes[i].Nodes[j].Nodes[k].Checked == true)
                        {
                            if (Branches != string.Empty)
                                Branches += ",";
                            Branches += tvBranches.Nodes[0].Nodes[i].Nodes[j].Nodes[k].Name.ToString();
                            iscomp = true;
                            iSstate = true;
                        }
                    ...
                    if (iSstate == true)
                    {
                        if (State != string.Empty)
                            State += ",";
                        State += tvBranches.Nodes[0].Nodes[i].Nodes[j].Name.ToString();
                    }
                    iSstate = false;
                }
                ...
                if (iscomp == true)
                {
                    if (Company != string.Empty)
                        Company += ",";
                    Company += tvBranches.Nodes[0].Nodes[i].Name.ToString();
                }
                iscomp = false;
            }
        }
