...
namespace SSCRM
{
    public partial class PUDeliveryChallanReceipt : Form
    {
        ...
        public PUDeliveryChallanReceipt()
        {
            InitializeComponent();
        }

        private void PUDeliveryChallanReceipt_Load(object sender, EventArgs e)
        {
        ...
        private void FillTransactionType()
        {
            ...
            try
            {
                ...
                cbTransactionType.SelectedValue = "PU2BR";
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            finally
            {
        ...
        private void FillBranchData()
        {
            ...
            {
                string sTrnType = "";
                 if (cbTransactionType.Text == "PU2BR")
                    sTrnType = "PU2BR";
        ...
        private string NewTransactionNumber()
        {
            ...
            try
            {
                sTranNo = objPUDB.GenerateNewDCRPUTranNo(CommonData.CompanyCode, CommonData.BranchCode, "DCR");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "DC PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
        ...
        {
            ProductSearchAll PSearch = new ProductSearchAll("DCR_PU");
            PSearch.objPUDeliveryChallanReceipt = this;
            PSearch.ShowDialog();
        }

        private void btnAddPMeterialItems_Click(object sender, EventArgs e)
        {
            ProductSearchAll PSearch = new ProductSearchAll("DCR_PU_PM");
            PSearch.objPUDeliveryChallanReceipt = this;
            PSearch.ShowDialog();
        }
        ...
        private bool CheckData()
        {
            ...
            {
                MessageBox.Show("Enter Reference No", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                blValue = false;
                txtRefNo.Focus();
                return blValue;
            }
            ...
            {
                MessageBox.Show("Enter valid  Date !", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                blValue = false;
            ...
            {
                MessageBox.Show("Date should be less than to day", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                dtpTranDate.CausesValidation = true;
                blValue = false;
                dtpTranDate.Focus();
                return blValue;
            ...
            if (chkCancelPuDCR.Checked == true)
            {
                if (txtRemarks.Text.Length <= 20)
                {
                    MessageBox.Show("Enter Remarks", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    blValue = false;
                    txtRemarks.Focus();
                    return blValue;
                }
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Select To Branch!", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    blValue = false;
                    cbBranches.Focus();
                    return blValue;
                }
                if (cbTransactionType.SelectedValue.ToString() == "PU2BR")
                {
                    ...
                    {
                        MessageBox.Show("Enter GL/GC number!", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        blValue = false;
                ...
                //{
                //    MessageBox.Show("Enter Freight !", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                //    blValue = false;
                ...
                if (cbVehicleType.Text != "BYHAND")
                {
                    ...
                    {
                        MessageBox.Show("Enter Trip / LR Number !", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        blValue = false;
                        txtTripLRNo.Focus();
                        return blValue;
                    }
                    ...
                    {
                        MessageBox.Show("Enter VehicleNo !", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        blValue = false;
                        txtVehicleNo.Focus();
                        return blValue;
                    }
                ...
                {
                    if (Convert.ToString(gvReqDetails.Rows[i].Cells["IssQty"].Value) != "" && Convert.ToString(gvReqDetails.Rows[i].Cells["IssQty"].Value) != "0")
                    {
                        blInvDtl = true;
                    }
                ...
                {
                    blValue = false;
                    MessageBox.Show("Enter Product Good Quantity", "DCR PU", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (CheckData())
            {
                try
                {
                    if (chkCancelPuDCR.Checked == true)
                    {
                        ...
                        if (intSave > 0)
                        {
                            ...
                            ClearForm(this);
                            txtTransactionNo.Text = NewTransactionNumber().ToString();
                            chkCancelPuDCR.Checked = false;
                            txtRemarks.Text = "";

                        }
                        else
                        {
                            MessageBox.Show("DCR Data not Saved.", "Delivery Challan PU", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show(ex.Message, "DCR", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
                finally
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    strSQL = " UPDATE PU_FG_DC_HEAD " +
                                "SET PUFDH_TRN_TYPE ='" + cbTransactionType.Text +
                                "', PUFDH_DOCUMENT_MONTH ='" + txtDocMonth.Text.ToUpper() +
                                "', PUFDH_TRN_DATE ='" + Convert.ToDateTime(dtpTranDate.Value).ToString("dd/MMM/yyyy") +
                                "', PUFDH_TO_BRANCH_CODE='" + ((SSCRM.ComboboxItem)(cbBranches.SelectedItem)).Value +
                                "', PUFDH_TO_ECODE=" + sEcode +
                                ", PUFDH_TRIP_OR_LR_NUMBER='" + txtTripLRNo.Text.Replace(" ", "") +
                                "', PUFDH_WAY_BILL_NO='" + txtWayBillNo.Text.Replace(" ", "") +
                                "', PUFDH_VEHICLE_SOURCE='" + cbVehicleType.Text +
                                "', PUFDH_VEHICLE_NUMBER='" + txtVehicleNo.Text.Replace(" ", "") +
        ...
        private int SaveDCDetlData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gvReqDetails.Rows.Count; i++)
                {
                    ...
                    if (iTotalQty != 0 ) //&& gvReqDetails.Rows[i].Cells["Amount"].Value.ToString() != "0.00")
                    {
                        sbSQL.Append("INSERT INTO PU_FG_DC_DETL (PUFDD_COMPANY_CODE "+
                                                              ", PUFDD_STATE_CODE "+
                                                              ", PUFDD_BRANCH_CODE "+
                                                              ", PUFDD_FIN_YEAR " +
                                                              ", PUFDD_TRN_TYPE "+
                                                              ...
                                                              ", PUFDD_BED_PED_PERC "+
                                                              ", PUFDD_BED_PED_VAL "+
                                                              ", PUFDD_EDCESS_PED_PERC "+
                                                              ", PUFDD_EDCESS_PED_VAL " +
                                                              ", PUFDD_SEC_EDCESS_PED_PERC "+
                                                              ", PUFDD_SEC_EDCESS_PED_VAL "+
                                                              ", PUFDD_AGNST_ST_FORM_TYPE "+
                                                              ", PUFDD_TAX_PERC "+
                                                              ", PUFDD_TAX_VAL " +
                                                              ", PUFDD_ISS_AMT "+
                                                              ", PUFDD_AGNST_DOC_REQ_TYPE "+
                                                              ", PUFDD_AGNST_DOC_REQ_NUMBER "+
                                                              ", PUFDD_AGNST_DOC_REQ_SL_NO)" +
                                                              ...
                                                              "', '" + cbTransactionType.Text + 
                                                              "', '" + txtTransactionNo.Text +
                                                              "', " + gvReqDetails.Rows[i].Cells["SLNO"].Value +
                                                              ", '" + gvReqDetails.Rows[i].Cells["BatchNo"].Value.ToString().Replace(" ", "") +
                                                              "', '" + gvReqDetails.Rows[i].Cells["ProductId"].Value.ToString() + 
                                                              "', " + gvReqDetails.Rows[i].Cells["IssQty"].Value +
                                                              ...
                                                              ",'', " + Convert.ToString(gvReqDetails.Rows[i].Cells["ReqNo"].Value) +
                                                              ", " + Convert.ToString(gvReqDetails.Rows[i].Cells["ReqSlno"].Value) + "); ");
        ...
        private int SaveDCPMDetlData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gvPackingDetl.Rows.Count; i++)
                {
                    ...
                    {
                        sbSQL.Append("INSERT INTO PU_PM_DC_DETL (PUPDD_COMPANY_CODE "+
                                                              ", PUPDD_STATE_CODE "+
                                                              ", PUPDD_BRANCH_CODE "+
                                                              ", PUPDD_FIN_YEAR " +
                                                              ", PUPDD_TRN_TYPE "+
                                                              ...
                                                              ", PUPDD_BED_PED_PERC "+
                                                              ", PUPDD_BED_PED_VAL "+
                                                              ", PUPDD_EDCESS_PED_PERC "+
                                                              ", PUPDD_EDCESS_PED_VAL " + 
                                                              ", PUPDD_SEC_EDCESS_PED_PERC "+
                                                              ", PUPDD_SEC_EDCESS_PED_VAL "+
                                                              ", PUPDD_AGNST_ST_FORM_TYPE "+
                                                              ", PUPDD_TAX_PERC "+
                                                              ", PUPDD_TAX_VAL " +
                                                              ", PUPDD_ISS_AMT "+
                                                              ", PUPDD_AGNST_DOC_REQ_TYPE "+
                                                              ", PUPDD_AGNST_DOC_REQ_NUMBER "+
                                                              ", PUPDD_AGNST_DOC_REQ_SL_NO)" +
                                                              " VALUES ('" + CommonData.CompanyCode +
                                                                    "', '" + CommonData.StateCode + 
                                                                    "', '" + CommonData.BranchCode +
                                                                    "', '" + CommonData.FinancialYear +
                                                                    "', '" + cbTransactionType.Text +
                                                                    "', '" + txtTransactionNo.Text +
                                                                    "', " + gvPackingDetl.Rows[i].Cells["SLNO1"].Value +
                                                                    ", '" + gvPackingDetl.Rows[i].Cells["BatchNo1"].Value +
                                                                    "', '" + gvPackingDetl.Rows[i].Cells["ProductId1"].Value + 
                                                                    "', " + gvPackingDetl.Rows[i].Cells["IssQty1"].Value +
        ...
        private void txtRefNo_Validated(object sender, EventArgs e)
        {
            ...
            {
                FillDCDetails(txtRefNo.Text.ToString());
            }
        }
        ...
        {
            double totGood = 0, totDmg = 0, totQty = 0, amt = 0;
            for (int i = 0; i < gvPackingDetl.Rows.Count; i++)
            {
                double iGood = 0, iDmg = 0, iTot = 0;
                if (Convert.ToString(gvPackingDetl.Rows[i].Cells["IssQty1"].Value).Replace(" ", "") != "")
                    iGood = Convert.ToDouble(gvPackingDetl.Rows[i].Cells["IssQty1"].Value.ToString().Replace(" ", ""));
                if (gvPackingDetl.Rows[i].Cells["IssDmgQty1"].Value.ToString().Replace(" ", "") != "")
                    iDmg = Convert.ToDouble(gvPackingDetl.Rows[i].Cells["IssDmgQty1"].Value.ToString().Replace(" ", ""));
                iTot = iGood + iDmg;
            ...
            for (int i = 0; i < gvReqDetails.Rows.Count; i++)
            {
                ...
                if (Convert.ToString(gvReqDetails.Rows[i].Cells["IssQty"].Value).Replace(" ", "") != "")
                ...
                if (Convert.ToString(gvReqDetails.Rows[i].Cells["IssQty"].Value) != "" && Convert.ToString(gvReqDetails.Rows[i].Cells["Rate"].Value) != "" && Convert.ToString(gvReqDetails.Rows[i].Cells["Amount"].Value) != "")
                {
            ...
            txtProducts.Text = Convert.ToDouble(gvReqDetails.Rows.Count.ToString()).ToString("f");
        ...
        private void gvPackingDetl_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > 0)
            {
                ...
                if (e.ColumnIndex == gvPackingDetl.Columns["Delete1"].Index)
                {
        ...
        private void gvReqDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == gvReqDetails.Columns["Delete"].Index)
            {
                DataGridViewRow tempRow = new DataGridViewRow();
        ...
        private void txt_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (Convert.ToString(tb.Tag) != "batchNo")
            {
        ...
        private bool FillTransactionHead(DataTable dtHead, DataTable dtDetl, DataTable dtPMDetl)
        {
            ...
            try
            {
                if (dtHead.Rows.Count > 0)
                {
                    ...
                    if (chkCancelPuDCR.Checked == false)
                    {
