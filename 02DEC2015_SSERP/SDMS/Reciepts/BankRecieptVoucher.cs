...
namespace SDMS
{
    public partial class BankRecieptVoucher : Form
    {
        ...
        public BankRecieptVoucher()
        {
            InitializeComponent();
        }
        public BankRecieptVoucher(string sCompCode, string sBranchCode, string sFinYear, string sDocType, string sVoucherId)
        {
            InitializeComponent();
        ...
        private void BankRecieptVoucher_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                        ...
                        try
                        {
                            ...
                            if (dtOutStanding.Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        int siNO = 0;
                                        foreach (DataRow tempDr in dts.Rows)
                                        {
                                            dts.Rows[siNO]["SL_NO"] = dtOutStanding.Rows.Count + 1;
                                            siNO++;
                                            dtOutStanding.ImportRow(tempDr);
                ...
                if(dtD2.Rows.Count>0)
                {
                    ...
                    for (int i = 0; i < dtD2.Rows.Count; i++)
                    {
                        ...
                        dtD2.Rows[i]["isinos"] = dtD2.Rows[i]["MASTER_SL_NO"];
                    }
        ...
        private void GetAccountTypes()
        {
            ...
            try
            {
                String strCmd = "SELECT AM_ACCOUNT_ID, AM_ACCOUNT_NAME AccountName FROM FA_ACCOUNT_MASTER WHERE AM_COMPANY_CODE='"+CommonData.CompanyCode+"' AND AM_ACCOUNT_TYPE_ID IN('BANK')";
        ...
        private void btnBillDetails_Click(object sender, EventArgs e)
        {
            if (CheckData())
            {
                BankBillRecievedDetails billReceivedDetails = new BankBillRecievedDetails();
                billReceivedDetails.objBankReceiptVoucher = this;
        ...
        public void GetBillDetails()
        {
            ...
            for (int i = 0; i < dtBillRecievedDetails.Rows.Count; i++)
            {
                ...
                DataGridViewCell cellMobNo = new DataGridViewTextBoxCell();
                cellMobNo.Value = dtBillRecievedDetails.Rows[i]["MobileNo"];
                tempRow.Cells.Add(cellMobNo);
            ...
            if (e.RowIndex >= 0)
            {
                if (gvBillDetails.Rows[e.RowIndex].Cells["Edit"].Value.ToString().Trim() != "")
                {
                    if (Convert.ToBoolean(gvBillDetails.Rows[e.RowIndex].Cells["Edit"].Selected) == true)
                    {
                        ...
                        BankBillRecievedDetails billReceivedDetails = new BankBillRecievedDetails(dr);
                        billReceivedDetails.objBankReceiptVoucher = this;
                        billReceivedDetails.ShowDialog();
                                ...
                                for (int j = 0; j < gvBillDetails.Rows.Count; j++)
                                {
                                    if (resultSms == DialogResult.Yes)
                                    {
                                        ...
                                        string sMessage = "We have recieved Rs." + gvBillDetails.Rows[j].Cells["AmtReceived"].Value.ToString() +
                                                    ...
                                                    " vide Reciept no " + gvBillDetails.Rows[j].Cells["Remarks"].Value.ToString() +
                                                    " on " + Convert.ToDateTime(gvBillDetails.Rows[j].Cells["Date"].Value.ToString()).ToString("dd-MMM-yyyy") +
                                                    " Reference " + gvBillDetails.Rows[j].Cells["PaymentMode"].Value.ToString() + " No " + gvBillDetails.Rows[j].Cells["ChqRefNo"].Value.ToString();
                                        //string sMessage = "Dealer Name: " + gvBillDetails.Rows[j].Cells["AccName"].Value.ToString();
