...
namespace SDMS
{
    public partial class BillReceivedDetails : Form
    {
        SQLDB objSQLdb;
        ...
        public ReceiptVoucher objReceiptVoucher;
        string strType = "";
        DataGridView dgvBillDetails = null;
        private DataTable dt = new DataTable();
        public bool isModifyFlag = false;
        private string sMobileNo = "";
        public BillReceivedDetails()
        {
            InitializeComponent();
        }

        public BillReceivedDetails(DataRow[] dr)
        {
            InitializeComponent();
            drs = dr;
        }

        private void BillReceivedDetails_Load(object sender, EventArgs e)
        {

            dt = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Clone();

            txtReceivedAmt.Text = "0";
            dtpBillDate.Value = DateTime.Today;
            cbAdjustmentType.SelectedIndex = 0;
            cbPaymentMode.SelectedIndex = 0;
            DataGridView dgvBillDetails = ((ReceiptVoucher)objReceiptVoucher).gvBillDetails;
        ...
        private void FillDetailData()
        {
            ...
            drss = ((ReceiptVoucher)objReceiptVoucher).dtAgnstVoucherBill.Select("isinos='" + drs[0][1] + "'");
            if (drss.Length > 0)
            {
                for (int i = 0; i < drss.Length; i++)
                {
                    DataRow[] tempRow = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Select("OU_BILL_NUMBER='" + drss[i][11] + "'");
                    if (tempRow.Length > 0)
                    {
        ...
        private bool CheckData()
        {
            ...
            {
                MessageBox.Show("Enter Account Number!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                flag = false;
            ...
            {
                MessageBox.Show("Select PaymentMode!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                flag = false;
                cbPaymentMode.Focus();
                return flag;
            }
            ...
            {
                MessageBox.Show("Enter Reference number!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                flag = false;
                txtChqNo.Focus();
                return flag;
            }
            ...
            {
                MessageBox.Show("Enter Received Amount!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                flag = false;
            ...
            {
                MessageBox.Show("Enter Amount!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if(CheckData())
            {
                ...
                if (drs != null)
                {
                    ...
                    ((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows.Remove(drs[0]);
                    for (int i = 0; i < drss.Length; i++)
                    {
                        ((ReceiptVoucher)objReceiptVoucher).dtAgnstVoucherBill.Rows.Remove(drss[i]);
                    }
                }
                else
                {
                ...
                try
                {
                    int i = 0;
                    int count = 0;
                    if (((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows.Count == 0)
                    {
                    ...
                    {
                        count = Convert.ToInt32(((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows[((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows.Count - 1]["SlNo"].ToString()) + 1;
                        i = Convert.ToInt32(((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows[((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows.Count - 1]["isinos"].ToString()) + 1;
                    }
                    ((ReceiptVoucher)objReceiptVoucher).dtBillRecievedDetails.Rows.Add(new Object[] { count ,
                    ...
                    objReceiptVoucher.txtVoucherId.Text,objReceiptVoucher.dtpVoucherDate.Value,"C","","",objReceiptVoucher.txtVoucherNo.Text,
                    objReceiptVoucher.cbAccType.SelectedValue.ToString(),sMobileNo});
                    ...
                    for (int iRow = 0; iRow < gvOutstandigDetails.Rows.Count; iRow++)
                    {
                        ...
                        //{
                            ...
                            ((ReceiptVoucher)objReceiptVoucher).dtAgnstVoucherBill.Rows.Add(new Object[] {i, objReceiptVoucher.txtVoucherId.Text, 
                                Convert.ToDateTime(objReceiptVoucher.dtpVoucherDate.Value).ToShortDateString(), count ,
                    ...
                    objReceiptVoucher.iSiNo = i;
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.ToString());
                }
                ((ReceiptVoucher)objReceiptVoucher).GetBillDetails();
                ((ReceiptVoucher)objReceiptVoucher).CaluculatingTotalRcvdAmt();
        ...
        private void UpdatingOutstandingTableBeforeSaving()
        {
            for (int i = 0; i < drss.Length; i++)
            {
                ...
                {
                    DataRow[] dr = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Select("OU_BILL_NUMBER='" + drss[i][11]+ "'");
                    if (dr.Length > 0)
                    {
                        if (Convert.ToDouble(dr[0][11].ToString()) > 0)
                        {
                            int iSNo = Convert.ToInt32(dr[0][0].ToString());
                            double OutStdgAmt = Convert.ToDouble(((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows[iSNo - 1]["OU_AMOUNT"].ToString());
                            double PaidAmt = Convert.ToDouble(((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows[iSNo - 1]["OU_AMT_PAID_RCVD"].ToString());
                            OutStdgAmt += Convert.ToDouble(drss[i][15]);

                            PaidAmt -= Convert.ToDouble(drss[i][15]);
                            ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows[iSNo - 1]["OU_AMOUNT"] = OutStdgAmt;
                            ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows[iSNo - 1]["OU_AMT_PAID_RCVD"] = PaidAmt;
                        }
        ...
        private void UpdatingOutstandingTable()
        {
            ...
            for (int i = 0; i < gvOutstandigDetails.Rows.Count; i++)
            {
                if (Convert.ToDouble(gvOutstandigDetails.Rows[i].Cells["ReceivedAmt"].Value.ToString()) > 0)
                {
                    ...
                    DataRow[] dr = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Select("OU_BILL_NUMBER='" + gvOutstandigDetails.Rows[i].Cells["InvoiceNumber"].Value + "'");
                    if (dr.Length > 0)
                    {
                        if (Convert.ToDouble(dr[0][11].ToString()) > 0)
                        {
        ...
        private void EcodeSearch()
        {
            ...
            try
            {
                ...
                //DataRow[] dtRow = dtEmp.Select("AM_ACCOUNT_ID=" + ((ReceiptVoucher)objReceiptVoucher).cbAccType.SelectedValue.ToString());
                //if (dtRow.Length > 0)
                //{
        ...
        private void txtAccountSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                if (txtAccountSearch.Text.ToString().Trim().ToUpper()  == ((ReceiptVoucher)objReceiptVoucher).cbAccType.SelectedValue.ToString())
                {
        ...
        public void FillOutstandingAmount(string sAccountId)
        {
            ...
            if (!CheckingForOutStandingAmtInTempTable(sAccountId))
            {
                ...
                foreach (DataRow dr in dts.Rows)
                {
                    ...
                    dts.Rows[i]["SL_NO"] = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows.Count + 1;
                    i++;
                    ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.ImportRow(dr);
        ...
        {
            bool flag = false;
            if (((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Rows.Count > 0 && sAccountId.Length>0 )
            {
                DataRow[] dr = ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Select("OU_ACCOUNT_ID='" + sAccountId +"'");
                dtTemp =  ((ReceiptVoucher)objReceiptVoucher).dtOutStanding.Clone();
                if (dr.Length > 0)
                {
                   flag = true;
        ...
        private void gvOutstandigDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 12)
            {
                ...
                {
                    gvOutstandigDetails.Rows[e.RowIndex].Cells["ReceivedAmt"].Value = "0";
                    MessageBox.Show("Enter Received Amount First!", "Cash Receipt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    txtReceivedAmt.Focus();
                }
            }
        }
